...
def check_ceph_status(args):
    ...
    if status_data['health']['overall_status'] != 'HEALTH_OK':
        ...
        try:
            ...
            pass
        try:
            if status_data['pgmap']['recovering_objects_per_sec'] < \
               args.recovery_rate:
                status_critical = True
                status_msg.append("Recovering objects/sec {}".format(
                    status_data['pgmap']['recovering_objects_per_sec']))
        except KeyError:
            pass
        if status_critical:
        ...
        if status_data['health']['overall_status'] == 'HEALTH_WARN':
            msg = "WARNING: ceph misplaced {}%, degraded {}%".format(
                  status_data['pgmap']['misplaced_ratio'] * 100,
                  status_data['pgmap']['degraded_ratio'] * 100)
            # TODO (XP) check for rebalance progress
...
if __name__ == '__main__':
    ...
    parser.add_argument('--recovery_rate', dest='recovery_rate',
                        default=10,
                        help="""
        Recovery rate below which we consider recovery to be stalled
                        """)
